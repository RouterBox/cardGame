...
// schemaSlug/packageSlug/toWire are optional overrides so the same client
// serves every content schema (character, race, world, lore-era, fount,
// world-narrative — RouterBox approved all six live syncs 2026-08-06). The
// defaults reproduce the original card behavior byte-for-byte, so existing
// callers and tests are unaffected. `toWire` maps a repo-side record to the
// schema's wire fields; the wire record's `name` remains the upsert identity
// key for every schema (world-narrative's toWire copies title into name for
// exactly this reason).
function createJainaClient({ apiKey, projectId, baseUrl, schemaSlug, packageSlug, toWire } = {}) {
  ...
  const resolvedSchemaSlug = schemaSlug || SCHEMA_SLUG;
  const resolvedPackageSlug = packageSlug || PACKAGE_SLUG;
  const resolvedToWire = toWire || toWireFields;
  const resolvedBaseUrl = baseUrl || process.env.JAINA_API_BASE_URL || DEFAULT_API_BASE_URL;
  const recordsUrl = `${resolvedBaseUrl}/projects/${encodeURIComponent(projectId)}/schemas/${encodeURIComponent(resolvedSchemaSlug)}/records`;
  const headers = {
  ...
  async function loadNameIndex() {
    if (!indexPromise) {
      indexPromise = (async () => {
        const url = `${recordsUrl}?package_slug=${encodeURIComponent(resolvedPackageSlug)}`;
  ...
  async function upsert(record) {
    ...
    const wire = resolvedToWire(record);
    const existingId = index.get(wire.name);
    const body = JSON.stringify({ package_slug: resolvedPackageSlug, ...wire });
    ...
    if (!res.ok) {
      ...
      throw new Error(
        `Jaina upsert failed for "${wire.name}" (${res.status} ${res.statusText}): ${responseBody}`
      );
    }

    const result = await res.json();
    if (!existingId && result && result.id) {
      index.set(wire.name, result.id);
    }
    return result;
  }

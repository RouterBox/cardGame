...
function createJainaClient({ apiKey, projectId, baseUrl } = {}) {
  ...
  const recordsUrl = `${resolvedBaseUrl}/projects/${encodeURIComponent(PROJECT_SLUG)}/schemas/${SCHEMA_SLUG}/records`;
  const headers = {
  ...
  async function upsert(record) {
    ...
    const body = JSON.stringify({ package_slug: PACKAGE_SLUG, ...record });

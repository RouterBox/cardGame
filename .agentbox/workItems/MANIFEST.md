...
## 2026-07-30-cardgame-jaina-character-sync-dryrun-cardgame-tool-dry-run-sync-of-character-records-into-jaina-content-ba.md

- AC1 [paraphrase]: node tools/sync-characters-to-jaina.js --dry-run exits 0 and prints exactly 20 JSON objects, one per character found across design/characters/cindral-reach.md, mireth-bloom.md, panoptic-concord.md, starweave-communion.md, and wrought-assembly.md (4 each), with design/characters/web.md excluded entirely — PASS
- AC2 [paraphrase]: Each printed record has name, slug, race, title, bio, and threads fields; slug is computed with the identical slugify(name) algorithm already used by lib/parse-card-markdown.js and tools/render-card.js, so the same character name produces the same slug anywhere else in the repo — PASS
- AC3 [inferred] (held_out): Each record's race field matches the source filename's race (e.g. every character parsed from design/characters/mireth-bloom.md has race 'Mireth Bloom'), and threads is an array of the character's '**Threads:**' bullet strings verbatim, or an empty array for a character with no Threads section — PASS
- AC4 [inferred]: Without --dry-run, the script makes no Jaina API calls, no network access, and no credentials are required by node --test — it prints a message that live sync is not yet implemented and exits 1, exactly mirroring tools/sync-cards-to-jaina.js's existing no-flag behavior — PASS
- AC5 [inferred]: Running the dry-run twice in a row against unchanged markdown produces byte-identical stdout output (deterministic: no timestamps, no randomness, stable character and field ordering matching each race file's on-disk order) — PASS

## 2026-07-30-cardgame-jaina-race-sync-dryrun-cardgame-tool-dry-run-sync-of-race-records-into-jaina-content-backbone-sli.md

- AC1 [paraphrase]: node tools/sync-races-to-jaina.js --dry-run exits 0 and prints exactly 5 JSON objects, one per file in design/races/: cindral-reach.md, mireth-bloom.md, panoptic-concord.md, starweave-communion.md, and wrought-assembly.md — PASS
- AC2 [paraphrase]: Each printed record has name, slug, identity, primaryStrength, complementaryStrengths, counteringWeaknesses, signatureHooks, and visualIdentity fields; slug is computed with the identical slugify(name) algorithm already used by lib/parse-card-markdown.js, so the same race name produces the same slug anywhere else in the repo — PASS
...
## 2026-07-30-cardgame-jaina-lore-sync-dryrun-cardgame-tool-dry-run-sync-of-lore-era-records-into-jaina-content-backbone.md

- AC1 [paraphrase]: node tools/sync-lore-eras-to-jaina.js --dry-run exits 0 and prints exactly 6 JSON objects, one per era heading in design/lore.md, in Timeline-of-Eras order: The Weave Age, The Sundering, The Long Dark, The Five Risings, The Cinderglass War, Current Era: The Uneasy Expanse — PASS
- AC2 [paraphrase]: Each printed record has name, slug, order, and summary fields; slug is computed with the identical slugify(name) function already exported by lib/parse-card-markdown.js, so the same era name produces the same slug anywhere else in the repo — PASS
...
## 2026-07-30-cardgame-jaina-sync-tools-dedup-extract-the-duplicated-dry-run-not-implemented-cli-boilerplate-shared-by-c.md

- AC1 [inferred]: lib/run-jaina-dryrun-cli.js exports a single function that both tools/sync-characters-to-jaina.js and tools/sync-races-to-jaina.js call from their main(), replacing each tool's own duplicated --dry-run check, not-implemented/exit-1 branch, and per-record JSON.stringify console.log loop — PASS
...
- AC3 [paraphrase]: test/sync-characters-to-jaina.test.js and test/sync-races-to-jaina.test.js pass unmodified against the refactored tools, including each suite's AC4 check that the tool's own source file contains no jaina-client/network/subprocess references — PASS
- AC4 [inferred] (held_out): Without --dry-run, each tool still exits 1 and prints its own tool-specific not-implemented message (mentioning 'characters' or 'races' by name, not a generic shared string that loses the per-tool wording), and still requires no JAINA_API_KEY/JAINA_PROJECT_ID in the environment — PASS
- AC5 [inferred]: lib/run-jaina-dryrun-cli.js has its own direct unit test exercising both branches (dry-run mode prints one JSON line per input record in order; non-dry-run mode prints the passed-in message and yields exit code 1) independent of either sync tool — PASS

## 2026-07-30-cardgame-playtest-spatial-race-identity-refresh-add-a-spatial-race-identity-worked-example-to-design-playt.md

- AC1 [inferred]: design/playtest-spatial.md's Procedure contains a new step or worked-example subsection naming all 5 Spatial Race Identity cards verbatim: Preemptive Survey, Unbound Passage, Chokepoint Garrison, Bloom Fount, Circuit Fount — no receipt (escalated before receipt computation)
- AC2 [paraphrase]: Each of the 5 cards' Cost line as narrated in the new text matches its Cost line exactly as printed in its source file (spatial-race-identity-set.md or spatial-race-identity-set-wave-2.md): Preemptive Survey 1 Signal, Unbound Passage 2 Tangle, Chokepoint Garrison 2 Mass, Bloom Fount 2 Bloom, Circuit Fount 2 Circuit — no receipt (escalated before receipt computation)
- AC3 [inferred] (held_out): The new text cites rules.md Section 8.3 for Preemptive Survey and Circuit Fount, Sections 8.4 and 8.6 for Unbound Passage, Section 8.5 for Chokepoint Garrison, and Section 4.6 for Bloom Fount — no receipt (escalated before receipt computation)
- AC4 [inferred]: The existing steps 1-10 procedure text, the Materials section, and the existing 'What to watch for' bullets are all still present unchanged (only additive changes: a new step/subsection and at least one new 'What to watch for' bullet) — no receipt (escalated before receipt computation)
- AC5 [paraphrase]: site/design/playtest-spatial.html is regenerated from the updated markdown via tools/build-site.js and contains the same 5 card names — no receipt (escalated before receipt computation)
...
## 2026-07-30-cardgame-playtest-character-signatures-wave-2-refresh-add-a-worked-example-to-design-playtest-full-game-md.md

- AC1 [inferred]: design/playtest-full-game.md contains a new 'Worked Example 4' section (following Worked Example 3) naming all 5 wave-2 cards verbatim: Torel Ashgrave, Line-Captain of the Ember Vanguard; Rathe Ossuary-Kin, Spore-Hound of the Sprawl; Doran Vex Amaranthine, Ledger-Warden of the Foreknowledge Archive; Ysolde Thane, Pilgrim of the Unwritten Sign; Foreman-Prime Yssa Ductile, Keeper of the First Pattern — no receipt (escalated before receipt computation)
- AC2 [paraphrase]: Each card's Cost line as narrated in the new section matches character-signatures-wave-2.md exactly: Torel Ashgrave 2 Mass, Rathe Ossuary-Kin 3 Bloom, Doran Vex Amaranthine 2 Signal, Ysolde Thane 2 Tangle, Foreman-Prime Yssa Ductile 4 Circuit — no receipt (escalated before receipt computation)
- AC3 [inferred] (held_out): The new section cites Section 4.2 in connection with Rathe Ossuary-Kin's Growth counter and Section 3 in connection with Doran Vex Amaranthine's and Ysolde Thane's Archive-reading abilities — no receipt (escalated before receipt computation)
- AC4 [inferred]: Step 1's two 40-card decklists, Section 11's copy-count discussion, and the existing text of Worked Examples 1, 2, and 3 are all present unchanged (only additive: the new Worked Example 4 section) — no receipt (escalated before receipt computation)
- AC5 [paraphrase]: site/design/playtest-full-game.html is regenerated from the updated markdown via tools/build-site.js and contains all 5 card names — no receipt (escalated before receipt computation)

## 2026-07-30-cardgame-lib-markdown-section-parser-dedup-extract-the-duplicated-h2-h3-section-splitting-and-paragraph-ex.md

- AC1 [inferred]: lib/markdown-sections.js exists and exports splitIntoH2Sections, splitIntoH3SectionsWithParent, and extractParagraph; lib/parse-race-markdown.js and lib/parse-lore-markdown.js both import splitIntoH2Sections from it instead of defining their own copy; lib/parse-star-atlas-markdown.js and lib/parse-founts-markdown.js both import splitIntoH3SectionsWithParent from it instead of defining their own copy — no receipt (escalated before receipt computation)
- AC2 [inferred]: All four parser files import extractParagraph from lib/markdown-sections.js instead of each defining its own version, and lib/parse-race-markdown.js and lib/parse-star-atlas-markdown.js both import slugify from lib/parse-card-markdown.js instead of redeclaring it locally — no receipt (escalated before receipt computation)
...
## 2026-07-30-cardgame-playtest-second-deck-pairing-write-a-second-full-game-playtest-procedure-design-playtest-full-gam.md

- AC1 [inferred]: design/playtest-full-game-2.md exists and contains two 40-card (or larger) decklists, each satisfying rules.md Section 11.1 (at least 40 cards) and Section 11.2 (no more than 3 copies of any single card Name) — no receipt (escalated before receipt computation)
- AC2 [inferred]: Across the two decklists combined, all 20 named cards from wormhole-restrictions-set.md, wormhole-closure-cards.md, spatial-race-identity-set.md, spatial-race-identity-set-wave-2.md, and character-signatures-wave-2.md appear at least once each, with each card's Cost line as printed in the decklist matching that card's Cost line exactly as printed in its own source file — no receipt (escalated before receipt computation)
...
## 2026-07-30-cardgame-frontier-worlds-cards-add-a-frontier-worlds-card-set-naming-halvorne-junction-kelmourn-drift-and.md

- AC1 [inferred]: design/cards/frontier-worlds-set.md exists with exactly 5 cards, one per race under design/races/, each following the Section 9.1 template field order (Name, Cost line, Type line, Rules text, flavor text) — no receipt (escalated before receipt computation)
- AC2 [inferred]: Across the 5 cards, the exact strings 'Halvorne Junction', 'Kelmourn Drift', and 'Tallowfen' each appear at least once — no receipt (escalated before receipt computation)
- AC3 [paraphrase] (held_out): Each card's Rules text names a Discovery-family or other Section 8 mechanic (Frontier Discovery, Contested Discovery, Neutral Planet, Blockade, or Wormhole Length) already defined in rules.md, not an invented mechanic — no receipt (escalated before receipt computation)
- AC4 [inferred]: design/DESIGN-READINESS.md's Section 3 contains exactly one new bullet citing 'frontier-worlds-set.md', and every other section of that file, design/star-atlas.md, and every other design/cards/*.md file are byte-for-byte unchanged — no receipt (escalated before receipt computation)
- AC5 [paraphrase]: site/design/cards/frontier-worlds-set.html exists and site/design/DESIGN-READINESS.html is regenerated via tools/build-site.js, and a new test file mechanically asserts all of the above — no receipt (escalated before receipt computation)

## 2026-07-30-cardgame-jaina-world-narrative-sync-dryrun-sync-design-world-md-s-the-setting-and-a-history-in-brief-narra.md

- AC1 [inferred]: lib/parse-world-narrative-markdown.js exports loadWorldNarrativeSections(), returning exactly 2 records with title values 'The Setting' and 'A History in Brief', in that order, each with a non-empty body string matching that section's extracted paragraph text from design/world.md — PASS
- AC2 [paraphrase] (held_out): The returned records never include a 'Cosmology: The Five Founts' entry, and lib/parse-founts-markdown.js's own loadAllFounts() output is unaffected — PASS
...
## 2026-07-31-cardgame-game-name-card-set-docs-propagate-wreck-tangle-into-all-10-existing-card-set-docs-the-one-target.md

- AC1 [inferred]: All 10 existing design/cards/*.md set files (alpha-set.md, frontier-set.md, character-signatures.md, character-signatures-wave-2.md, character-signatures-wave-3.md, spatial-race-identity-set.md, spatial-race-identity-set-wave-2.md, wormhole-restrictions-set.md, wormhole-closure-cards.md, fount-economy-set.md) contain the string "Wreck Tangle" at least once, added inside each file's existing "## Summary" section — no receipt (escalated before receipt computation)
- AC2 [inferred]: Every "###" card entry (Name, Cost line, Type line, Rules text, flavor text) and every file's H1 title, across all 10 files, are byte-for-byte unchanged from their content before this unit — no receipt (escalated before receipt computation)
